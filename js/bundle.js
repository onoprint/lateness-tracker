...
`)}generateDailySheet(t,e){let s=this.classManager.getById(t);if(!s)return null;let a=this.studentManager.getSortedByName(t),i=this.storage.get("arrivals",[]).filter(n=>n.classId===t&&n.date===e);return{classId:t,className:s.name,date:e,students:a.map(n=>{let r=i.find(d=>d.studentId===n.id);return{id:n.id,name:n.name,photoUrl:n.photoUrl,arrived:!!r,time:r?r.time:null,minutesLate:r?r.minutesLate:0,status:r?r.status:"absent"}})}}exportJSON(){return JSON.stringify(this.storage.exportAll(),null,2)}importJSON(t){try{let e=JSON.parse(t);return this.storage.importAll(e),{success:!0}}catch(e){return{success:!1,error:e.message}}}};var y=class{constructor(t){this.app=t,this.currentClassId=null,this.currentDate=new Date().toISOString().split("T")[0],this.views={}}init(){this.renderApp(),this.setupEventListeners(),this.showClassSelector()}renderApp(){let t=document.getElementById("app");t.innerHTML=`
        ...
        `}setupEventListeners(){document.getElementById("datePicker").addEventListener("change",t=>{this.currentDate=t.target.value,this.refreshCurrentView()}),document.getElementById("bottomNav").addEventListener("click",t=>{if(t.target.classList.contains("nav-btn")){let e=t.target.dataset.view;this.switchView(e)}}),document.getElementById("menuBtn").addEventListener("click",()=>{this.toggleSidebar()}),document.getElementById("modal").addEventListener("click",t=>{t.target.id==="modal"&&this.closeModal()})}showClassSelector(){let t=this.app.classManager.getAll(),e=document.getElementById("classNav");t.length===0?e.innerHTML=`
                <button class="add-class-btn" onclick="window.app.ui.showAddClassModal()">
                    + Add Class
                </button>
            `:(e.innerHTML=t.map(s=>`
                <button class="class-btn ${this.currentClassId===s.id?"active":""}" 
                        data-class-id="${s.id}">
                    ${s.name}
                </button>
            `).join(""),e.querySelectorAll(".class-btn").forEach(s=>{s.addEventListener("click",()=>{this.selectClass(s.dataset.classId)})}),this.currentClassId||this.selectClass(t[0].id))}selectClass(t){this.currentClassId=t,document.querySelectorAll(".class-btn").forEach(e=>{e.classList.toggle("active",e.dataset.classId===t)}),this.refreshCurrentView()}switchView(t){document.querySelectorAll(".nav-btn").forEach(s=>{s.classList.toggle("active",s.dataset.view===t)});let e=document.getElementById("mainContent");switch(t){case"tracker":this.renderTrackerView(e);break;case"students":this.renderStudentsView(e);break;case"schedule":this.renderScheduleView(e);break;case"reports":this.renderReportsView(e);break}this.currentView=t}renderTrackerView(t){if(!this.currentClassId){t.innerHTML='<p class="empty-state">Select a class first</p>';return}let e=this.app.studentManager.getSortedByName(this.currentClassId),s=this.app.arrivalTracker.getByClassAndDate(this.currentClassId,this.currentDate),a=this.app.classManager.getById(this.currentClassId),i=new Date(this.currentDate).getDay(),n=this.app.classManager.getScheduleForDay(this.currentClassId,i);t.innerHTML=`
            <div class="tracker-view">
                <div class="tracker-header">
                    <h2>${a.name}</h2>
                    <p class="date-display">${this.formatDate(this.currentDate)}</p>
                    ${n&&n.enabled?`<p class="schedule-info">Class: ${n.startTime} - ${n.endTime}</p>`:'<p class="no-class">No class scheduled</p>'}
                </div>
                
                <div class="students-grid">
                    ${e.map(r=>{let d=s.find(o=>o.studentId===r.id);return this.renderStudentCard(r,d)}).join("")}
                </div>
                
                <div class="tracker-stats">
                    <span>Present: ${s.length}</span>
                    <span>Absent: ${e.length-s.length}</span>
                </div>
            </div>
        `,t.querySelectorAll(".student-card").forEach(r=>{r.addEventListener("click",()=>{this.toggleArrival(r.dataset.studentId)})})}renderStudentCard(t,e){let s=!!e,a=s?e.status:"absent",i=s?e.minutesLate>0?`${e.minutesLate}min late`:"On time":"Absent";return`
            <div class="student-card ${a}" data-student-id="${t.id}">
                <div class="student-photo">
                    ${t.photoUrl?`<img src="${t.photoUrl}" alt="${t.name}">`:'<div class="photo-placeholder">\u{1F464}</div>'}
                </div>
                <div class="student-name">${t.name}</div>
                <div class="student-status">${i}</div>
                ${s?`<div class="arrival-time">${e.time}</div>`:""}
            </div>
        `}toggleArrival(t){this.app.arrivalTracker.hasArrived(t,this.currentDate)?this.app.arrivalTracker.removeArrival(t,this.currentDate):this.app.arrivalTracker.markArrival(t,this.currentClassId,this.currentDate),this.refreshCurrentView()}renderStudentsView(t){if(!this.currentClassId){t.innerHTML='<p class="empty-state">Select a class first</p>';return}let e=this.app.studentManager.getSortedByName(this.currentClassId);t.innerHTML=`
            <div class="students-view">
                ...
                ${e.length===0?'<p class="empty-state">No students yet</p>':""}
            </div>
        `}renderScheduleView(t){if(!this.currentClassId){t.innerHTML='<p class="empty-state">Select a class first</p>';return}let e=this.app.classManager.getById(this.currentClassId),s=["monday","tuesday","wednesday","thursday","friday","saturday","sunday"],a=["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"];t.innerHTML=`
            <div class="schedule-view">
                <h2>Schedule - ${e.name}</h2>
                
                <div class="schedule-grid">
                    ${s.map((i,n)=>{let r=e.schedule[i]||{enabled:!1};return`
                            <div class="schedule-day ${r.enabled?"enabled":""}">
                                <label class="day-name">
                                    <input type="checkbox" ${r.enabled?"checked":""} 
                                           onchange="window.app.ui.toggleDay('${i}')">
                                    ${a[n]}
                                </label>
                                ${r.enabled?`
                                    <div class="time-inputs">
                                        <input type="time" value="${r.startTime||"08:00"}"
                                               onchange="window.app.ui.updateSchedule('${i}', 'startTime', this.value)">
                                        <span>to</span>
                                        <input type="time" value="${r.endTime||"12:00"}"
                                               onchange="window.app.ui.updateSchedule('${i}', 'endTime', this.value)">
                                    </div>
        ...
        `}renderReportsView(t){let e=this.app.classManager.getById(this.currentClassId),s=new Date;t.innerHTML=`
            <div class="reports-view">
                <h2>Reports - ${e?.name||"Select Class"}</h2>
                
                <div class="report-options">
                    <label>
                        Month:
                        <select id="reportMonth">
                            ${Array.from({length:12},(a,i)=>`<option value="${i+1}" ${i+1===s.getMonth()+1?"selected":""}>
                    ...
                    <label>
                        ...
                        <select id="reportYear">
                            ${Array.from({length:3},(a,i)=>`<option value="${s.getFullYear()-i}" ${i===0?"selected":""}>
                                    ${s.getFullYear()-i}
                                </option>`).join("")}
                        </select>
                    </label>
        ...
        `}showAddClassModal(){this.showModal(`
            <h2>Add Class</h2>
        ...
        `)}addClass(t){t.preventDefault();let s=t.target.className.value;this.app.classManager.add(s),this.showClassSelector(),this.closeModal()}showAddStudentModal(){this.showModal(`
            <h2>Add Student</h2>
        ...
        `)}addStudent(t){t.preventDefault();let e=t.target,s=e.studentName.value,a=e.photoUrl.value||null;this.app.studentManager.add(s,this.currentClassId,a),this.refreshCurrentView(),this.closeModal()}showImportModal(){this.showModal(`
        ...
        `)}importStudents(t){t.preventDefault();let s=t.target.csvContent.value,a=this.app.studentManager.importFromCSV(s,this.currentClassId);alert(`Imported ${a.imported} students${a.errors.length>0?". Errors: "+a.errors.join(", "):""}`),this.refreshCurrentView(),this.closeModal()}deleteStudent(t){confirm("Delete this student?")&&(this.app.studentManager.delete(t),this.refreshCurrentView())}toggleDay(t){let e=this.app.classManager.getById(this.currentClassId),s=e.schedule[t]?.enabled||!1;this.app.classManager.update(this.currentClassId,{schedule:{...e.schedule,[t]:s?{enabled:!1}:{enabled:!0,startTime:"08:00",endTime:"12:00"}}}),this.refreshCurrentView()}updateSchedule(t,e,s){let a=this.app.classManager.getById(this.currentClassId);this.app.classManager.update(this.currentClassId,{schedule:{...a.schedule,[t]:{...a.schedule[t],[e]:s}}})}async generatePDFReport(){let t=parseInt(document.getElementById("reportMonth").value),e=parseInt(document.getElementById("reportYear").value),s=this.app.reportGenerator.generateMonthlyReport(this.currentClassId,e,t),a=this.generateReportHTML(s),i=window.open("","_blank");i.document.write(a),i.document.close(),i.print()}generateCSVReport(){let t=parseInt(document.getElementById("reportMonth").value),e=parseInt(document.getElementById("reportYear").value),s=this.app.reportGenerator.generateMonthlyReport(this.currentClassId,e,t),a=this.app.reportGenerator.generateCSV(s),i=new Blob([a],{type:"text/csv"}),n=URL.createObjectURL(i),r=document.createElement("a");r.href=n,r.download=`rapport_${s.className}_${t}_${e}.csv`,r.click(),URL.revokeObjectURL(n)}generateReportHTML(t){return`
            ...
            <body>
                <h1>Rapport de Retards - ${t.className}</h1>
                <p class="text-center">${t.monthName} ${t.year}</p>
                <table>
                    <thead>
                        <tr>
                            <th>\xC9l\xE8ve</th>
                    ...
                    <tbody>
                        ${t.students.map(e=>`
                            <tr>
                                ...
                                <td class="text-center">${e.onTime}</td>
                                <td class="text-center">${e.tardies}</td>
                                <td class="text-right">${e.totalMinutesLate}</td>
                                <td class="text-right">${e.avgMinutesLate}</td>
                            </tr>
                        `).join("")}
                    </tbody>
                </table>
                <p style="margin-top: 20px; text-align: right;">
                    G\xE9n\xE9r\xE9 le ${new Date().toLocaleDateString()}
                </p>
            </body>
            </html>
        `}showModal(t){let e=document.getElementById("modal");e.innerHTML=`<div class="modal-content">${t}</div>`,e.classList.remove("hidden")}closeModal(){document.getElementById("modal").classList.add("hidden")}toggleSidebar(){document.getElementById("sidebar").classList.toggle("hidden")}refreshCurrentView(){this.switchView(this.currentView||"tracker")}formatDate(t){return new Date(t).toLocaleDateString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}};var f=class{constructor(t="lateness-tracker"){this.namespace=t}set(t,e){try{let s=JSON.stringify(e);return localStorage.setItem(`${this.namespace}:${t}`,s),!0}catch(s){return console.error(`Storage error: ${t}`,s),!1}}get(t,e=null){try{let s=localStorage.getItem(`${this.namespace}:${t}`);return s?JSON.parse(s):e}catch(s){return console.error(`Storage read error: ${t}`,s),e}}remove(t){localStorage.removeItem(`${this.namespace}:${t}`)}exportAll(){let t={};for(let e=0;e<localStorage.length;e++){let s=localStorage.key(e);if(s.startsWith(`${this.namespace}:`)){let a=s.replace(`${this.namespace}:`,"");t[a]=this.get(a)}}return t}importAll(t){Object.entries(t).forEach(([e,s])=>{this.set(e,s)})}clearAll(){let t=[];for(let e=0;e<localStorage.length;e++){let s=localStorage.key(e);s.startsWith(`${this.namespace}:`)&&t.push(s)}t.forEach(e=>localStorage.removeItem(e))}};var S=class{constructor(){this.storage=new f,this.classManager=new m(this.storage),this.studentManager=new g(this.storage),this.arrivalTracker=new v(this.storage,this.classManager),this.reportGenerator=new b(this.storage,this.studentManager,this.classManager),this.ui=new y(this),this.init()}async init(){console.log("\u{1F4F1} Lateness Tracker starting..."),await this.classManager.load(),await this.studentManager.load(),await this.arrivalTracker.load(),this.ui.init(),console.log("\u2705 App initialized")}getState(){return{classes:this.classManager.getAll(),students:this.studentManager.getAll(),arrivals:this.arrivalTracker.getAll(),currentClassId:this.ui.currentClassId,currentDate:this.ui.currentDate}}};document.addEventListener("DOMContentLoaded",()=>{window.app=new S});})();
